...
<content>
    <style type="text/css">
        #lan-grid .co1, #lan-grid .co2, #lan-grid .co3, #lan-grid .co4, #lan-grid
        .co5, #lan-grid .co6, #lan-grid .co7 { text-align: center; } #lan-grid
        .centered { text-align: center; } #spin { visibility: hidden; vertical-align:
        middle; }
    </style>
    ...
    <script type='text/javascript'>
        ...
        lg.setup = function() {
            ...
            for (var i = 0; i <= MAX_BRIDGE_ID; i++) {
                ...
                if (nvram['lan' + j + '_ifname'].length > 0) {
                    ...
                    lg.insertData( - 1, [i.toString(), nvram['lan' + j + '_stp'], nvram['lan' + j + '_ipaddr'], nvram['lan' + j + '_netmask'], (nvram['lan' + j + '_proto'] == 'dhcp') ? '1': '0', nvram['dhcpd' + j + '_startip'], nvram['dhcpd' + j + '_endip'], (nvram['lan' + j + '_proto'] == 'dhcp') ? (((nvram['dhcp' + j + '_lease']) * 1 == 0) ? '1440': (nvram['dhcp' + j + '_lease']).toString()) : '']);
                    numBridges++;
                }
            }
            lg.canDelete = false;
        ...
        lg.dataToView = function(data) {
            return ['br' + data[0], (data[1].toString() == '1') ? '<small><i>启用</i></small>': '<small><i>禁用</i></small>', data[2], data[3], (data[4].toString() == '1') ? '<small><i>启用</i></small>': '<small><i>禁用</i></small>', (data[5].toString() + ((numberOfBitsOnNetMask(data[3]) >= 24) ? (' - ' + data[6].split('.').splice(3, 1).toString()) : ('<br>' + data[6].toString()))), (((data[7] != null) && (data[7] != '')) ? data[7] : '')];
        }
        ...
        lg.verifyFields = function(row, quiet) {
            ...
            if ((f[4].checked) && (v_ip(f[2], 1)) && (v_netmask(f[3], 1))) {
                ...
                if (f[5].value == '') {
                    ...
                    var o = (m) ^ (~aton(f[3].value)) var n = o - m;
                    do {
                        if (--n < 0) {
                            f[5].value = '';
                            return;
                ...
                // first IP valid?
                if ((getNetworkAddress(f[5].value, f[3].value) != getNetworkAddress(f[2].value, f[3].value)) || (f[5].value == getBroadcastAddress(getNetworkAddress(f[2].value, f[3].value), f[3].value)) || (f[5].value == getNetworkAddress(f[2].value, f[3].value)) || (f[2].value == f[5].value)) {
                ...
                // last IP valid?
                if ((getNetworkAddress(f[6].value, f[3].value) != getNetworkAddress(f[2].value, f[3].value)) || (f[6].value == getBroadcastAddress(getNetworkAddress(f[2].value, f[3].value), f[3].value)) || (f[6].value == getNetworkAddress(f[2].value, f[3].value)) || (f[2].value == f[6].value)) {
                ...
                // lease time
                if (parseInt(f[7].value * 1) == 0) f[7].value = 1440; // from nvram/defaults.c
                if (!v_mins(f[7], quiet || !ok, 1, 10080)) ok = 0;
            } else {
                f[5].disabled = 1;
                f[6].disabled = 1;
                f[7].disabled = 1;
        ...
        function refreshNetModes(uidx) {
            ...
            for (i = 0; i < m.length; ++i) buf += '<option value="' + m[i][0] + '"' + ((m[i][0] == val) ? ' selected': '') + '>' + m[i][1] + '</option>';
            ...
            refresher[uidx].onCompleted = function(text, xml) {
                try {
                    ...
                    for (i = 0; i < wl_channels.length; ++i) {
                        ghz[uidx].push([wl_channels[i][0] + '', (wl_channels[i][0]) ? ((wl_channels[i][1]) ? wl_channels[i][0] + ' - ' + (wl_channels[i][1] / 1000.0).toFixed(3) + ' GHz': wl_channels[i][0] + '') : '自动']);
                    ...
                    for (i = 0; i < ghz[uidx].length; ++i) buf += '<option value="' + ghz[uidx][i][0] + '"' + ((ghz[uidx][i][0] == val) ? ' selected': '') + '>' + ghz[uidx][i][1] + '</option>';
            ...
            xob.onCompleted = function(text, xml) {
                try {
                    ...
                    for (i = 0; i < wlscandata.length; ++i) {
                        ...
                        if (!wscan.seen[mac]) {
                            wscan.seen[mac] = 1; ++wscan.inuse[ch].count;
                        }
        ...
        function verifyFields(focused, quiet) {
            ...
            for (uidx = 1; uidx <= maxwan_num; ++uidx) {
                ...
                vis['_wan' + u + '_ppp_idletime'] = (E('_wan' + u + '_ppp_demand').value == 1) && vis['_wan' + u + '_ppp_demand'] vis['_wan' + u + '_ppp_redialperiod'] = !vis['_wan' + u + '_ppp_idletime'] && vis['_wan' + u + '_ppp_demand'];
            ...
            for (i = a.length - 1; i >= 0; --i) if ((vis[a[i]]) && (!v_dns(a[i], quiet || !ok))) ok = 0;
        ...
        function earlyInit() {
            var mwan = E('_mwan_num');
            if (nvram.wan_ifnameX.length < 1) mwan.options[0].disabled = true;
            if (nvram.wan2_ifnameX.length < 1) mwan.options[1].disabled = true;
            /* MULTIWAN-BEGIN */
            if (nvram.wan3_ifnameX.length < 1) mwan.options[2].disabled = true;
            if (nvram.wan4_ifnameX.length < 1) mwan.options[3].disabled = true;
            /* MULTIWAN-END */
        ...
        function save() {
            ...
            if ((fom['lan_ifname'].value != 'br0') || (fom['lan_ipaddr'].value == '0.0.0.0') || (!t)) {
                e.innerHTML = '网口LAN0必须始终定义，并设置有效的IP地址.';
                e.style.visibility = 'visible';
                setTimeout(function() {
                    e.innerHTML = '';
                    e.style.visibility = 'hidden';
    ...
    <form id="_fom" method="post" action="tomato.cgi">
        ...
        <div class="box" data-box="network-wan">
            <div class="heading" id="mwan-title">
                多拨设置
            </div>
            <div class="content" id="wan-settings">
            </div>
            /* REMOVE-BEGIN */
            ...
            <script type="text/javascript">
                ...
                $('#wan-settings').forms([{
                    ...
                    name: 'mwan_num',
                    type: 'select',
                    options: [['1', '1 WAN'], ['2', '2 WAN']
                    /* MULTIWAN-BEGIN */
                    , ['3', '3 WAN'], ['4', '4 WAN']
                    /* MULTIWAN-END */
                    ],
                    value: nvram.mwan_num,
                    suffix: ' <small>请先设置 <a href="/#advanced-vlan.asp">VLAN</a></small>'
                },
                {
                    title: '检查连接',
                    name: 'mwan_cktime',
                    type: 'select',
                    options: [['0', '禁用'], ['60', '1 分钟'], ['120', '2 分钟（默认）'], ['180', '3 分钟'], ['300', '5 分钟'], ['600', '10 分钟'], ['900', '15 分钟'], ['1800', '30 分钟'], ['3600', '1 小时']],
            ...
            <script type="text/javascript">
                ...
                for (var uidx = 1; uidx <= maxwan_num; ++uidx) {
                    ...
                    $('#wan-settings').forms([{
                        ...
                        name: 'wan' + u + '_proto',
                        type: 'select',
                        options: [['dhcp', '动态IP'], ['static', '静态IP'], ['pppoe', '宽带拨号'], ['pptp', 'PPTP客户端'], ['l2tp', 'L2TP客户端'],
                        /* LINUX26-BEGIN */
                        /* USB-BEGIN */
                        ['ppp3g', '3G 调制解调器设备'], ['lte', '4G/LTE'],
                        /* USB-END */
                        /* LINUX26-END */
                        ['disabled', '禁用']],
                    ...
                    },
                    {
                        ...
                        maxlen: 3,
                        size: 8,
                        value: nvram['wan' + u + '_weight'],
                        suffix: ' <i>(故障转移：0; 负载均衡：1 - 256)</i>'
                    },
                    {
                        title: 'PIN码',
                        name: 'wan' + u + '_modem_pin',
                        type: 'text',
                        maxlen: 6,
                        size: 8,
                        value: nvram['wan' + u + '_modem_pin'],
                        suffix: ' <i>建议关闭PIN码</i>'
                    },
                    {
                        title: '调制解调器初始化参数',
                        name: 'wan' + u + '_modem_init',
                        type: 'text',
                        maxlen: 25,
                        size: 32,
                        value: nvram['wan' + u + '_modem_init']
                    },
                    {
                        title: 'APN',
                        name: 'wan' + u + '_modem_apn',
                        type: 'text',
                        maxlen: 25,
                        size: 32,
                        value: nvram['wan' + u + '_modem_apn']
                    },
                    {
                        title: '网络类型',
                    ...
                    {
                        title: '用户名',
                        name: 'wan' + u + '_ppp_username',
                        type: 'text',
                        maxlen: 60,
                        size: 64,
                        value: nvram['wan' + u + '_ppp_username']
                    },
                    {
                        title: '密码',
                        name: 'wan' + u + '_ppp_passwd',
                        type: 'password',
                        maxlen: 60,
                        size: 64,
                        peekaboo: 1,
                        value: nvram['wan' + u + '_ppp_passwd']
                    },
                    {
                        title: '服务名称',
                        name: 'wan' + u + '_ppp_service',
                        type: 'text',
                        maxlen: 50,
                        size: 64,
                        value: nvram['wan' + u + '_ppp_service']
                    },
                    {
                        title: 'L2TP服务器',
                        name: 'wan' + u + '_l2tp_server_ip',
                        type: 'text',
                        maxlen: 128,
                        size: 64,
                        value: nvram['wan' + u + '_l2tp_server_ip']
                    },
                    {
                        title: '使用DHCP服务',
                        name: 'f_wan' + u + '_pptp_dhcp',
                        type: 'checkbox',
                        value: (nvram['wan' + u + '_pptp_dhcp'] == 1)
                    },
                    {
                        title: 'IP地址',
                        name: 'wan' + u + '_ipaddr',
                    ...
                    },
                    {
                        ...
                        maxlen: 15,
                        size: 17,
                        value: nvram['wan' + u + '_gateway']
                    },
                    {
                        title: 'PPTP网关',
                        name: 'wan' + u + '_pptp_server_ip',
                        type: 'text',
                        maxlen: 128,
                        size: 64,
                        value: nvram['wan' + u + '_pptp_server_ip']
                    },
                    {
                        title: '选项',
                        name: 'wan' + u + '_ppp_custom',
                        type: 'text',
                        maxlen: 256,
                        size: 64,
                        value: nvram['wan' + u + '_ppp_custom']
                    },
                    {
                        title: 'DNS服务器',
                        name: 'wan' + u + '_dns_auto',
                        type: 'select',
                        options: [['1', '自动'], ['0', '自定义']],
                        value: nvram['wan' + u + '_dns_auto']
                    },
                    {
                        title: 'DNS 1',
                        indent: 2,
                    ...
                    {
                        title: '连接模式',
                        name: 'wan' + u + '_ppp_demand',
                        type: 'select',
                        options: [['1', '按需连接'], ['0', '自动连接']],
                        value: nvram['wan' + u + '_ppp_demand']
                    },
                    {
                        title: '自动断线空闲时间',
                        indent: 2,
                        name: 'wan' + u + '_ppp_idletime',
                        type: 'text',
                        maxlen: 5,
                        size: 7,
                        suffix: ' <i>(分钟)</i>',
                    ...
                    },
                    {
                        title: 'LCP回波间隔',
                        indent: 2,
                        name: 'wan' + u + '_pppoe_lei',
                        type: 'text',
                        maxlen: 5,
                        size: 7,
                        suffix: ' <i>秒 (范围：1-60; 默认值：10)</i>',
                        value: nvram['wan' + u + '_pppoe_lei']
                    },
                    {
                        title: 'LCP回显链路故障限制',
                        indent: 2,
                        name: 'wan' + u + '_pppoe_lef',
                        type: 'text',
                        maxlen: 5,
                        size: 7,
                        suffix: ' <i>(范围：1-10; 默认值：5)</i>',
                        value: nvram['wan' + u + '_pppoe_lef']
                    },
                    {
                        title: 'MTU值',
                        multi: [{
                            name: 'wan' + u + '_mtu_enable',
                            type: 'select',
                            options: [['0', '默认'], ['1', '自定义']],
                            value: nvram['wan' + u + '_mtu_enable']
                        },
                        {
                            name: 'f_wan' + u + '_mtu',
                            type: 'text',
                            maxlen: 4,
                            size: 6,
                            value: nvram['wan' + u + '_mtu']
                        }]
                    },
                    {
                        title: '单线多拨',
                        name: 'f_wan' + u + '_ppp_mlppp',
                        type: 'checkbox',
                        value: (nvram['wan' + u + '_ppp_mlppp'] == 1)
                    },
                    {
                        title: '路由调制解调器IP',
                        ...
                        maxlen: 15,
                        size: 17,
                        suffix: ' <i>(必须在不同的子网到路由器，0.0.0.0 代表禁用)</i>',
                    ...
                    },
                    {
                        ...
                        name: 'wan' + u + '_ckmtd',
                        type: 'select',
                        options: [['1', 'Ping'], ['2', '跟踪路由*']],
                        value: nvram['wan' + u + '_ckmtd'],
                        suffix: ' <i>(默认：跟踪路由; 仅当Traceroute不正常工作时，才使用Ping</i>'
                    },
                    {
                        title: '桥接WAN端口到主LAN口(LAN0)',
                        name: 'f_wan' + u + '_islan',
                        type: 'checkbox',
                        value: (nvram['wan' + u + '_islan'] == 1)
                    }]);
        ...
        <div class="box" data-box="network-lan">
            ...
            <div class="content lan-settings">
                <!-- VLAN-BEGIN -->
                <table class="line-table" id="lan-grid">
                </table>
                <br />
                <script type="text/javascript">
                    lg.setup();
            ...
            <script type="text/javascript">
                ...
                {
                    title: '启用DNSSEC',
                    name: 'f_dnssec_enable',
                    type: 'checkbox',
                    suffix: ' <i>(必须由上游名称服务器支持)</i>',
                    value: (nvram.dnssec_enable == 1)
                },
                /* DNSSEC-END */
                /* DNSCRYPT-BEGIN */
                {
                    title: '使用 dnscrypt-proxy',
                    name: 'f_dnscrypt_proxy',
                    type: 'checkbox',
                    value: (nvram.dnscrypt_proxy == 1)
                },
                {
                    title: '手动输入',
                    indent: 2,
                    name: 'f_dnscrypt_manual',
                    type: 'checkbox',
                    value: (nvram.dnscrypt_manual == 1)
                },
                {
                    title: 'DNS服务器',
                    indent: 2,
                    name: 'dnscrypt_resolver',
                    type: 'select',
                    options: [
                    /*dnscrypt_resolvers*/
                    ],
                    value: nvram.dnscrypt_resolver,
                    suffix: ' <a href=\'https://github.com/jedisct1/dnscrypt-proxy/blob/master/dnscrypt-resolvers.csv\' target=\'_new\'>DNS详细信息</a>'
                },
                {
                    title: 'DNS服务器地址',
                    indent: 2,
                    name: 'dnscrypt_resolver_address',
                    type: 'text',
                    maxlen: 50,
                    size: 25,
                    value: nvram.dnscrypt_resolver_address,
                    suffix: ' <a href=\'https://github.com/jedisct1/dnscrypt-proxy/blob/master/dnscrypt-resolvers.csv\' target=\'_new\'>DNS详细信息</a>'
                },
                {
                    title: 'DNS服务器名称',
                    ...
                    maxlen: 60,
                    size: 25,
                    value: nvram.dnscrypt_provider_name
                },
                {
                    title: '提供的公钥',
                    indent: 2,
                    name: 'dnscrypt_provider_key',
                    type: 'text',
                    maxlen: 80,
                    size: 25,
                    value: nvram.dnscrypt_provider_key
                },
                {
                    title: '优先级',
                    indent: 2,
                    name: 'dnscrypt_priority',
                    type: 'select',
                    options: [['1', 'Strict-Order'], ['2', 'No-Resolv'], ['0', 'None']],
                    value: nvram.dnscrypt_priority
                },
                {
                    title: '本地端口',
                    indent: 2,
                ...
                },
                {
                    ...
                    maxlen: 2,
                    size: 5,
                    value: nvram.dnscrypt_log
                },
                /* DNSCRYPT-END */
                {
                    title: 'WINS <i>(用于DHCP服务)</i>',
                    name: 'wan_wins',
                    type: 'text',
                    maxlen: 15,
                    size: 17,
                    value: nvram.wan_wins
                }]);
            </script>
        </div>
        ...
        <script type="text/javascript">
            ...
            for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
                ...
                if (wl_sunit(uidx) < 0) {
                    ...
                    {
                        title: 'MAC地址',
                        text: '<a href="/#advanced-mac.asp">' + eval('nvram.wl' + u + '_hwaddr') + '</a>'
                    },
                    {
                        title: '无线模式',
                        name: 'f_wl' + u + '_mode',
                    ...
                    },
                    {
                        ...
                        options: [],
                        prefix: '<span id="__wl' + u + '_net_mode">',
                        suffix: '</span>'
                    },
                    {
                        title: 'SSID',
                    ...
                    },
                    {
                        ...
                        name: 'f_wl' + u + '_bcast',
                        type: 'checkbox',
                        value: (eval('nvram.wl' + u + '_closed') == '0')
                    },
                    {
                        title: '信道',
                        name: 'wl' + u + '_channel',
                        type: 'select',
                        options: ghz[uidx],
                        prefix: '<div class="pull-left" id="__wl' + u + '_channel">',
                        suffix: '</div> &nbsp; <button class="btn btn-primary" type="button" id="_f_wl' + u + '_scan" value="Scan" onclick="scanButton(' + u + ')">扫描 <i class="icon-search"></i></button> &nbsp; <div class="spinner" style="visibility: hidden;" id="spin' + u + '"></div>',
                    ...
                    },
                    {
                        ...
                        prefix: '<span id="__wl' + u + '_nbw_cap">',
                        suffix: '</span>'
                    },
                    {
                        title: '控制边带',
                        name: 'f_wl' + u + '_nctrlsb',
                        type: 'select',
                        options: [['lower', '下层'], ['upper', '上层']],
                        value: eval('nvram.wl' + u + '_nctrlsb') == 'none' ? 'lower': eval('nvram.wl' + u + '_nctrlsb')
                    },
                    {
                        title: '安全模式',
                        name: 'wl' + u + '_security_mode',
                        type: 'select',
                    ...
                    },
                    {
                        ...
                        name: 'wl' + u + '_crypto',
                        type: 'select',
                        options: [['tkip', 'TKIP'], ['aes', 'AES'], ['tkip+aes', 'TKIP / AES']],
                        value: eval('nvram.wl' + u + '_crypto')
                    },
                    {
                        title: '共享密钥',
                        indent: 2,
                    ...
                    },
                    {
                        ...
                        indent: 2,
                        multi: [{
                            name: 'wl' + u + '_radius_ipaddr',
                            type: 'text',
                            maxlen: 15,
                            size: 17,
                            value: eval('nvram.wl' + u + '_radius_ipaddr')
                        },
                        {
                            name: 'wl' + u + '_radius_port',
                            type: 'text',
                            maxlen: 5,
                            size: 7,
                            prefix: ' : ',
                            value: eval('nvram.wl' + u + '_radius_port')
                        }]
                    },
                    {
                        title: '加密类型',
                        ...
                        name: 'wl' + u + '_wep_bit',
                        type: 'select',
                        options: [['128', '128-bits'], ['64', '64-bits']],
                        value: eval('nvram.wl' + u + '_wep_bit')
                    },
                    {
                        title: '密码',
                        indent: 2,
                        name: 'wl' + u + '_passphrase',
                        type: 'text',
                        maxlen: 16,
                        size: 20,
                    ...
                    f.push(null, {
                        title: 'WDS',
                        name: 'f_wl' + u + '_lazywds',
                        type: 'select',
                        options: [['0', '链接...'], ['1', '自动']],
                        value: nvram['wl' + u + '_lazywds']
                    });
                    ...
                    for (i = 0; i < 10; i += 2) {
                        f.push({
                            ...
                            multi: [{
                                ...
                                maxlen: 17,
                                size: 20,
                                value: wds[i] || '00:00:00:00:00:00'
                            },
                            {
                                name: 'f_wl' + u + '_wds_' + (i + 1),
                                type: 'text',
                                maxlen: 17,
                                size: 20,
                                value: wds[i + 1] || '00:00:00:00:00:00'
                            }]
                        });
                    }
