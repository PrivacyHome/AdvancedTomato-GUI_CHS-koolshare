...
function systemUI() {
	...
	$.ajax(
		...
		$( '.system-ui .datasystem' ).html(
			...
			'<div class="desc">CPU负载:</div><div class="value">' + stats.cpuload + '</div>' +
			'<div class="desc">RAM占用:</div><div class="value">' + stats.memory + '<div class="progress small"><div class="bar" style="width: ' + stats.memoryperc + '"></div></div></div>' +
			((nvram.swap != null) ? '<div class="desc">SWAP占用:</div><div class="value">' + stats.swap + '<div class="progress small"><div class="bar" style="width: ' + stats.swapperc + '"></div></div></div>' : '') +
			'<div class="desc ">WAN状态:</div><div class="value">' + wanstatus + ' (' + stats.wanuptime[ 0 ] + ')</div></div>' ).removeClass( 'align center'
		);
