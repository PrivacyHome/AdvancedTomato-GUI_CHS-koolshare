...
<content>
    ...
    <script type="text/javascript">
        ...
        function verifyFields(focused, quiet) {
            ...
            if (focused) {
                ...
                if (clientindex >= 0) {
                    ...
                    if (stripped == 'vpn_client_local') E('_f_vpn_client' + clientnumber + '_local').value = focused.value;
                    else if (stripped == 'f_vpn_client_local') E('_vpn_client' + clientnumber + '_local').value = focused.value;
        ...
        RouteGrid.prototype.verifyFields = function(row, quiet) {
            ...
            if (f[2].value == "") {
                ferror.set(f[2], "值是必需的.", quiet);
                ret = 0;
            }
            if (f[2].value.indexOf('>') >= 0 || f[2].value.indexOf('<') >= 0) {
                ferror.set(f[2], "值不能包含 '<' 或 '>' 符号。", quiet);
                ret = 0;
            }
            if (f[2].value.indexOf(' ') >= 0 || f[2].value.indexOf(',') >= 0) {
                ferror.set(f[2], "值不能包含 '空格' 或 ',' 符号。 每个条目只有一个IP或域。", quiet);
                ret = 0;
            }
            if (f[2].value.indexOf(' ') >= 0) {
                ferror.set(f[2], "值不能包含 '-' 符号。 不支持的IP范围。", quiet);
                ret = 0;
            }
            return ret;
        }
        ...
        RouteGrid.prototype.dataToView = function(data) {
            var temp = ['<div class="checkbox c-checkbox"><label><input type=\'checkbox\' style="opacity:1" disabled' + (data[0] != 0 ? ' checked': '') + '> <span></span></label></div>', ['从源IP', '到目标IP', '到域'][data[1] - 1], data[2]];
            var v = [];
        ...
        function save() {
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                if (E('_f_vpn_' + t + '_eas').checked) E('vpn_client_eas').value += '' + (i + 1) + ',';

                var routedata = routingTables[i].getAllData();
                var routing = '';
                for (j = 0; j < routedata.length; ++j) routing += routedata[j].join('<') + '>';
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                if (routingVal.length) {
                    ...
                    for (var j = 0; j < s.length; ++j) {
                        ...
                        var row = s[j].split('<');
                        if (row.length == 3) routingTables[i].insertData( - 1, row);
                    }
                }
    ...
    <form id="_fom" method="post" action="tomato.cgi">
        ...
        <script type="text/javascript">
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                htmlOut += '</ul>' + '<div class="box"><div class="heading">VPN 客户端 #' + (i + 1) + status + ' <a id="_vpn' + t + '_button" class="pull-right" href="#" data-toggle="tooltip"' + 'onclick="toggle(\'vpn' + t + '\', vpn' + (i + 1) + 'up); return false;"' + action + '</a></div><div class="content">';
                htmlOut += ('<div id=\'' + t + '-basic\'>');
                htmlOut += createFormFields([{
                    title: '连接互联网后运行',
                    name: 'f_vpn_' + t + '_eas',
                    type: 'checkbox',
                    value: nvram.vpn_client_eas.indexOf('' + (i + 1)) >= 0
                },
                {
                    title: '接口类型',
                    name: 'vpn_' + t + '_if',
                ...
                },
                {
                    ...
                    name: 'vpn_' + t + '_br',
                    type: 'select',
                    options: [['br0', 'LAN (br0)*'], ['br1', 'LAN1 (br1)'], ['br2', 'LAN2 (br2)'], ['br3', 'LAN3 (br3)']],
                    value: eval('nvram.vpn_' + t + '_br'),
                    suffix: ' <small>* 默认</small> '
                },
                {
                    title: '协议',
                    name: 'vpn_' + t + '_proto',
                    type: 'select',
                    options: [['udp', 'UDP'], ['tcp-client', 'TCP']],
                ...
                {
                    title: '用户名/密码验证',
                    name: 'f_vpn_' + t + '_userauth',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_userauth') != 0
                },
                {
                    title: '用户名: ',
                    indent: 2,
                    name: 'vpn_' + t + '_username',
                    type: 'text',
                    maxlen: 50,
                    size: 54,
                    value: eval('nvram.vpn_' + t + '_username')
                },
                {
                    title: '密码: ',
                    indent: 2,
                    name: 'vpn_' + t + '_password',
                    type: 'password',
                    maxlen: 50,
                    size: 54,
                    value: eval('nvram.vpn_' + t + '_password')
                },
                {
                    title: '仅用户名验证',
                    indent: 2,
                    name: 'f_vpn_' + t + '_useronly',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_useronly') != 0,
                ...
                {
                    title: '服务器在同一子网上',
                    name: 'f_vpn_' + t + '_bridge',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_bridge') != 0,
                ...
                },
                {
                    ...
                    name: 'f_vpn_' + t + '_nat',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_nat') != 0,
                ...
                },
                {
                    ...
                    },
                    {
                        ...
                        maxlen: 15,
                        size: 17,
                        value: eval('nvram.vpn_' + t + '_gw'),
                        prefix: '<span id=\'' + t + '_gateway\'> 网关:&nbsp',
                        suffix: '</span>'
                    }]
                },
                {
                    title: '忽略重定向网关<br>（路由未推送）',
                    name: 'f_vpn_' + t + '_nopull',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_nopull') != 0
                },
                {
                    title: '接受DNS配置',
                    name: 'vpn_' + t + '_adns',
                    type: 'select',
                    options: [[0, '禁用'], [1, '宽松'], [2, '严格'], [3, '唯一']],
                    value: eval('nvram.vpn_' + t + '_adns')
                },
                {
                    title: '加密方式',
                    name: 'vpn_' + t + '_cipher',
                ...
                },
                {
                    ...
                    maxlen: 5,
                    size: 7,
                    value: eval('nvram.vpn_' + t + '_retry'),
                    suffix: '&nbsp;<small>(分钟; -1 为无限制)</small>'
                },
                {
                    title: '验证服务器证书（tls-remote）',
                    multi: [{
                        name: 'f_vpn_' + t + '_tlsremote',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_tlsremote') != 0
                    },
                    {
                        name: 'vpn_' + t + '_cn',
                        type: 'text',
                        maxlen: 64,
                        size: 54,
                        value: eval('nvram.vpn_' + t + '_cn'),
                        prefix: '<span id=\'' + t + '_cn\'> 公用名:&nbsp',
                        suffix: '</span>'
                    }]
                },
                {
                ...
                htmlOut += createFormFields([{
                    title: '通过VPN重定向',
                    name: 'f_vpn_' + t + '_route',
                    type: 'checkbox',
                    value: eval('nvram.vpn_' + t + '_route') != 0
                },
                {
                    title: '',
                    suffix: '</span><table class=\'tomato-grid line-table\' id=\'table_' + t + '_routing\'></table><span>'
                }]);
                htmlOut += '<div><ul><li><b>类型 -> 从源IP</b> - 例如: "1.2.3.4" 或 "1.2.3.0/24".'htmlOut += '<li><b>类型 -> 到目标IP</b> - 例如: "1.2.3.4" 或 "1.2.3.0/24".';
