...
<content>
    <style type='text/css'>
        #th-grid .co1 { width: 10%; text-align: center; } #th-grid .co2 { width:
        17%; } #th-grid .co3 { width: 29%; } #th-grid .co4 { width: 10%; } #th-grid
        .co5 { width: 14%; } #th-grid .co6 { width: 20%; } .editor td:first-child
        { text-align: center; } textarea { width: 100%; height: 10em; }
    </style>
    <script type="text/javascript">
        ...
        th.verifyFields = function(row, quiet) {
            ...
            if (f[0].checked && f[2].value == "") {
                ferror.set(f[2], "选中连接到时必须提供地址。", quiet);
                return 0;
            } else {
                ferror.clear(f[2])
            }
            ...
            if (E('_tinc_devicetype').value == 'tun') {
                if ((!v_subnet(f[5], 1)) && (!v_ip(f[5], 1))) {
                    ferror.set(f[5], "子网或IP地址无效。", quiet);
                    return 0;
                } else {
                    ferror.clear(f[5])
                }
                ...
                if (f[5].value != '') {
                    ferror.set(f[5], "使用TAP接口类型时，子网为空。", quiet);
                    return 0;
                } else {
                    ferror.clear(f[5])
                }
            ...
            if (E('_host_ed25519_key').value == "") {
                ferror.set(E('_host_ed25519_key'), "Ed25519 公钥是必需的。", quiet);
                return 0;
            } else {
                ferror.clear(E('_host_ed25519_key'))
            }
        ...
        function verifyFields(focused, quiet) {
            ...
            if (E('_tinc_name').value == "" && E('_f_tinc_wanup').checked) {
                ferror.set(E('_tinc_name'), "选中“启动WAN”时需要使用主机名。", quiet);
                return 0;
            } else {
            ...
            if (E('_tinc_private_ed25519').value == "" && E('_tinc_custom').value == "" && E('_f_tinc_wanup').checked) {
                ferror.set(E('_tinc_private_ed25519'), "Ed25519 选中“启动WAN”时需要私钥。", quiet);
                return 0;
            } else {
            ...
            if (!hostdefined && E('_f_tinc_wanup').checked) {
                ferror.set(E('_tinc_name'), "主机名 \"" + E('_tinc_name').value + "\" 必须在选中“启动WAN”时在主机区域中定义。", quiet);
                return 0;
            } else {
                ferror.clear(E('_tinc_name'))
            };
        ...
        function displayNodes() {
            ...
            var hostselect = document.getElementById("hostselect") var selected = hostselect.value;
        ...
        function toggle(service, isup) {
            ...
            if (nvram.tinc_hosts != s) changed = 1;
...
<div class="content">'
                ...
                html += createFormFields([{
                    ...
                    name: 'f_tinc_wanup',
                    type: 'checkbox',
                    value: (nvram.tinc_wanup == 1)
                },
                {
                    title: '接口类型',
                    name: 'tinc_devicetype',
                    type: 'select',
                    options: [['tun', 'TUN'], ['tap', 'TAP']],
                    value: nvram.tinc_devicetype
                },
                {
                    title: '模式',
                    name: 'tinc_mode',
                    type: 'select',
                    options: [['switch', '交换'], ['hub', '中转']],
                    value: nvram.tinc_mode
                },
                {
                    title: 'VPN网络掩码',
                    name: 'tinc_vpn_netmask',
                    type: 'text',
                    maxlen: 15,
                    size: 25,
                    value: nvram.tinc_vpn_netmask,
                    suffix: ' <small>整个VPN网络的网络掩码。</small>'
                },
                {
                    title: '主机名称',
                    name: 'tinc_name',
                    ...
                    maxlen: 30,
                    size: 25,
                    value: nvram.tinc_name,
                    suffix: ' <small>也必须在 \'主机\' 中定义。</small>'
                },
                {
                    title: 'Ed25519 私钥',
                    name: 'tinc_private_ed25519',
                    type: 'textarea',
                    value: nvram.tinc_private_ed25519
                },
                {
                    title: 'RSA 私钥 *',
                    name: 'tinc_private_rsa',
                    type: 'textarea',
                    value: nvram.tinc_private_rsa
                },
                {
                    title: '自定义',
                    name: 'tinc_custom',
                    type: 'textarea',
                    value: nvram.tinc_custom
                }]);
                html += '</div>';
                // -------- END CONFIG TAB -----------
                // -------- BEGIN HOSTS TAB -----------
                ...
                html += createFormFields([{
                    title: '防火墙规则',
                    name: 'tinc_manual_firewall',
                    type: 'select',
                    options: [['0', '自动'], ['1', '附加'], ['2', '自定义']],
                    value: nvram.tinc_manual_firewall
                },
                {
                    title: '防火墙',
                    name: 'tinc_firewall',
                    type: 'textarea',
                    value: nvram.tinc_firewall
                },
                {
                    title: 'tinc-up创建',
                    name: 'tinc_manual_tinc_up',
                    type: 'select',
                    options: [['0', '自动'], ['1', '自定义']],
                    value: nvram.tinc_manual_tinc_up
                },
                {
                    title: 'tinc-up',
                    name: 'tinc_tinc_up',
                    type: 'textarea',
                    value: nvram.tinc_tinc_up
                },
                {
                    title: 'tinc-down',
                    name: 'tinc_tinc_down',
                    type: 'textarea',
                    value: nvram.tinc_tinc_down
                },
                {
                    title: 'host-up',
                    name: 'tinc_host_up',
                    type: 'textarea',
                    value: nvram.tinc_host_up
                },
                {
                    title: 'host-down',
                    name: 'tinc_host_down',
                    type: 'textarea',
                    value: nvram.tinc_host_down
                },
                {
                    title: 'subnet-up',
                    name: 'tinc_subnet_up',
                    type: 'textarea',
                    value: nvram.tinc_subnet_up
                },
                {
                    title: 'subnet-down',
                    name: 'tinc_subnet_down',
                    type: 'textarea',
                    value: nvram.tinc_subnet_down
                }]);
                html += '</div>';
                // -------- END SCRIPTS TAB -----------
                // -------- BEGIN KEYS TAB -----------
